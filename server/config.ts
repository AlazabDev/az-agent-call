...
function required(name: string): string {
  ...
  if (!value) throw new Error(`[az-agent-call] Missing required environment variable ${name}`);
  return value;
}

const supabaseUrl = required("SUPABASE_URL").replace(/\/$/, "");
const inferredProjectRef = new URL(supabaseUrl).hostname.split(".")[0];
const supabaseProjectRef = (process.env.SUPABASE_PROJECT_REF ?? inferredProjectRef).trim();

export const config = {
  version: "3.0.1",
  port: Number(process.env.PORT ?? 3300),
  publicAppUrl: (process.env.PUBLIC_APP_URL ?? "https://agent-call.alazab.com").replace(/\/$/, ""),
  adminBasePath: "/admin",
  adminPassword: required("ADMIN_PASSWORD"),
  mailboxDomain: (process.env.MAILBOX_DOMAIN ?? "alazab.com").trim().toLowerCase(),
  ...
  agentTokensPath: path.resolve(process.env.AGENT_TOKENS_PATH ?? "/var/lib/az-agent-call/data/agent-tokens.json"),
  ...
  corsOrigins: (process.env.CORS_ORIGINS ?? `${(process.env.PUBLIC_APP_URL ?? "https://agent-call.alazab.com").replace(/\/$/, "")},http://localhost:8080`).split(",").map((value) => value.trim()).filter(Boolean),
  ...
  // Codex agent (Azure AI Foundry) — optional. Left unset, the "codex" MCP
  // connector reports itself as not-configured instead of blocking startup,
  // the same pattern used for the telephony provider below.
