...
export const config = {
  ...
  publicAppUrl: (process.env.PUBLIC_APP_URL ?? "https://mcp.alazab.com").replace(/\/$/, ""),
  adminBasePath: "/admin",
  ...
  agentTokensPath: path.resolve(process.env.AGENT_TOKENS_PATH ?? "/app/data/agent-tokens.json"),
  ...
  corsOrigins: (process.env.CORS_ORIGINS ?? `${(process.env.PUBLIC_APP_URL ?? "https://mcp.alazab.com").replace(/\/$/, "")},http://localhost:8080`).split(",").map((value) => value.trim()).filter(Boolean),
