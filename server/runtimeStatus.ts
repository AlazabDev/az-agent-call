...
async function heartbeat(): Promise<void> {
  ...
  const { error } = await supabaseAdmin.from("call_gateway_instances").upsert({
...
export async function touchAgentConnection(agentId: AgentId, tool: string, whoami = false): Promise<void> {
  const now = new Date().toISOString();
  const { data } = await supabaseAdmin.from("call_agent_connections").select("request_count").eq("agent_id", agentId).maybeSingle();
  ...
  const { error } = await supabaseAdmin.from("call_agent_connections").upsert(row, { onConflict: "agent_id" });
