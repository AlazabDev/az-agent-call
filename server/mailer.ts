...
async function writeSendLog(input: {
  ...
  if (error) console.error("[az-agent-call] failed to persist send log:", error.message);
}
...
export async function verifyAgent(agent: RuntimeAgent): Promise<{ ok: true } | { ok: false; error: string }> {
  try {
