...
export async function syncTemplateCatalogToSupabase(): Promise<number> {
	...
	for (let index = 0; index < rows.length; index += batchSize) {
		...
		if (error)
			throw new Error(
				`Template sync failed at batch ${index / batchSize + 1}: ${error.message}`,
			);
	}
	...
	const { data: existing, error: existingError } = await supabaseAdmin
		...
		.select("id");
	if (existingError)
		throw new Error(`Template stale-check failed: ${existingError.message}`);
	...
	if (staleIds.length) {
		const { error: staleError } = await supabaseAdmin
			...
			.in("id", staleIds);
		if (staleError)
			throw new Error(`Template stale-disable failed: ${staleError.message}`);
	}
