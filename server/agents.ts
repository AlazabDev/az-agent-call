...
import { agentIdForClearToken, hashAgentToken, rotateStoredAgentToken } from "./tokenStore.js";
import { touchAgentConnection } from "./runtimeStatus.js";

export type RuntimeAgent = AgentRecord;
export { hashAgentToken };

export async function agentForToken(token: string): Promise<RuntimeAgent | null> {
  const id = agentIdForClearToken(token);
  if (!id) return null;
  const agent = await getAgent(id);
  if (agent?.enabled) await touchAgentConnection(id, "mcp_request").catch(() => undefined);
...
export async function getAgent(id: string): Promise<RuntimeAgent | null> {
  ...
  const { data, error } = await supabaseAdmin
    .from("mail_agents")
    .select("id, foundry_id, mailbox, enabled, token_hint, token_rotated_at, smtp_password_env")
    .eq("id", id)
    .maybeSingle();
...
export async function rotateAgentToken(id: AgentId): Promise<{ token: string; hint: string }> {
...
export function smtpConfigured(agent: RuntimeAgent): boolean {
  return Boolean(process.env[agent.smtp_password_env]?.trim() || config.smtpPasswordPattern);
}

export function smtpPasswordSource(agent: RuntimeAgent): "override" | "pattern" | "missing" {
...
export function smtpPassword(agent: RuntimeAgent): string {
  ...
  if (!fallback) throw new Error(`SMTP password is not configured for ${agent.id}`);
  return fallback;
}
