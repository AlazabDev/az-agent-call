...
const _daftraBaseUrl =
	process.env.DAFTRA_BASE_URL?.trim() ||
	`https://${daftraSubdomain}.daftra.com/api2`;

export const registerDaftraConnector: McpConnectorRegister = (
	server: McpServer,
	agent: RuntimeAgent,
) => {
	server.registerTool(
		"daftra_get_client",
		{
			...
			description:
				"Searches and retrieves a client account profile from Daftra ERP by phone, email, client ID, or name.",
			inputSchema: {
		...
		async ({ phone, email, client_id, search }) => {
			try {
				await touchAgentConnection(agent.id, "daftra_get_client").catch(
					() => undefined,
				);
				const _query = phone || email || client_id || search || "عام";
				return {
					content: [
						{
							type: "text",
	...
	server.registerTool(
		"daftra_create_invoice",
		{
			...
			description:
				"Generates a new sales or service invoice in Daftra ERP for a client.",
			inputSchema: {
					...
					.array(
						z.object({
							item_id: z
								.string()
								.optional()
								.describe("Daftra product/service ID"),
							description: z
								.string()
								.min(1)
								.describe("Item or service description"),
				...
				notes: z
					.string()
					.optional()
					.describe("Invoice notes or call reference"),
				payment_status: z
					.enum(["draft", "unpaid", "paid"])
					.default("unpaid")
					.describe("Initial payment status"),
			},
			annotations: {
				readOnlyHint: false,
		...
		async ({ client_id, items, notes, payment_status }) => {
			try {
				await touchAgentConnection(agent.id, "daftra_create_invoice").catch(
					() => undefined,
				);
	...
	server.registerTool(
		"daftra_list_invoices",
		{
			...
			description:
				"Lists client invoices from Daftra filtered by client, status, or date.",
			inputSchema: {
		...
		async ({ client_id, status, limit }) => {
			try {
				await touchAgentConnection(agent.id, "daftra_list_invoices").catch(
					() => undefined,
				);
				return {
					content: [
	...
	server.registerTool(
		...
		async ({ invoice_id, amount, payment_method, transaction_reference }) => {
			try {
				await touchAgentConnection(agent.id, "daftra_record_payment").catch(
					() => undefined,
				);
				return {
					content: [
	...
	server.registerTool(
		"daftra_check_inventory",
		{
			...
			description:
				"Checks product/service stock level, pricing, and availability in Daftra ERP.",
			inputSchema: {
		...
		async ({ search }) => {
			try {
				await touchAgentConnection(agent.id, "daftra_check_inventory").catch(
					() => undefined,
				);
				return {
					content: [
