...
export const registerTelephonyConnector: McpConnectorRegister = (
	server: McpServer,
	agent: RuntimeAgent,
) => {
	server.registerTool(
		"whoami",
		{
			...
			description:
				"Returns the authenticated Foundry agent identity, fixed line/extension, telephony readiness, and MCP endpoint status.",
			inputSchema: {},
			annotations: { readOnlyHint: true, openWorldHint: false },
		},
		async () => {
			await touchAgentConnection(agent.id, "whoami", true).catch(
				() => undefined,
			);
			return {
				content: [
	...
	server.registerTool(
		"make_call",
		{
			...
			description:
				"Initiates an outbound voice call or audio session from the authenticated agent line.",
			inputSchema: {
				to: z
					.string()
					.min(3)
					.max(50)
					.describe("Recipient phone number or SIP extension"),
				subject: z
					.string()
					.min(1)
					.max(300)
					.describe("Call purpose or script subject"),
				template_id: z
					.string()
					.optional()
					.describe("Optional voice script template ID"),
			},
			annotations: {
				readOnlyHint: false,
		...
		async ({ to, subject, template_id }) => {
			try {
				await touchAgentConnection(agent.id, "make_call").catch(
					() => undefined,
				);
						...
						content: [
							{
								...
								text: JSON.stringify({
									...
									error:
										"Telephony provider (SIP/Twilio) is not configured in environment.",
								}),
							},
						],
					};
	...
	server.registerTool(
		"get_call_transcript",
		{
			...
			description:
				"Returns the text transcript and audio logs for a specific call session ID.",
			inputSchema: { call_session_id: z.string().min(1).max(200) },
			annotations: { readOnlyHint: true, openWorldHint: false },
		},
		async ({ call_session_id }) => {
			try {
				await touchAgentConnection(agent.id, "get_call_transcript").catch(
					() => undefined,
				);
				...
				const transcript =
					await telephonyService.getTranscript(call_session_id);
				return {
					content: [
						{
							type: "text",
