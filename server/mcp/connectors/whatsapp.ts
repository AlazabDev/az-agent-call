...
export const registerWhatsAppConnector: McpConnectorRegister = (
	server: McpServer,
	agent: RuntimeAgent,
) => {
	server.registerTool(
		"whatsapp_send_message",
		{
			...
			description:
				"Sends a direct text message or follow-up note via WhatsApp Business Cloud API to a client phone number.",
			inputSchema: {
				recipient_phone: z
					.string()
					.min(1)
					.describe(
						"Client WhatsApp phone number (e.g. +201115723930 or 201115723930)",
					),
				message_text: z.string().min(1).describe("Text message content"),
				daftra_client_id: z
					.number()
					.optional()
					.describe("Associated Daftra Client ID"),
			},
			annotations: {
				readOnlyHint: false,
		...
		async ({ recipient_phone, message_text, daftra_client_id }) => {
			try {
				await touchAgentConnection(agent.id, "whatsapp_send_message").catch(
					() => undefined,
				);
	...
	server.registerTool(
		"whatsapp_send_invoice_notification",
		{
			title: "WhatsApp Messaging: Send invoice & payment link template",
			description:
				"Sends a WhatsApp template message containing invoice details, total amount, and payment link.",
			inputSchema: {
				recipient_phone: z
					.string()
					.min(1)
					.describe("Client WhatsApp phone number"),
				client_name: z.string().min(1).describe("Client name"),
				invoice_number: z
					.string()
					.min(1)
					.describe("Invoice number (e.g. INV-2026-884)"),
				total_amount: z
					.string()
					.min(1)
					.describe("Total amount string (e.g. 450 EGP)"),
	...
	server.registerTool(
		"whatsapp_list_available_lines",
		{
			title: "WhatsApp Messaging: List available WhatsApp lines",
			description:
				"Returns the catalog of active WhatsApp lines, assigned WABAs, and agent roles.",
			inputSchema: {},
