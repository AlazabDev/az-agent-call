...
export function registerDaftraResolverTools(
	server: McpServer,
	agent: RuntimeAgent,
) {
	if (
		hasCapability(agent, "daftra.clients.read") ||
		hasCapability(agent, "daftra.products.read")
	) {
		server.registerTool(
			"daftra_search_entities",
			{
				...
				description:
					"Searches across clients, suppliers, products, work orders, cost centers, and treasuries with Arabic & English transliteration support.",
				inputSchema: daftraSearchEntitiesSchema.shape,
			},
			async (args) => {
				const start = Date.now();
				try {
					const matches = await daftraService.resolver.searchEntities(
						args.query,
						args.entity_type,
					);
