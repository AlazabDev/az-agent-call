...
export function registerDaftraProductTools(
	server: McpServer,
	agent: RuntimeAgent,
) {
	if (hasCapability(agent, "daftra.products.read")) {
		server.registerTool(
			"daftra_list_products",
			{
				...
				description:
					"Lists products and inventory items from Daftra ERP with optional search query.",
				inputSchema: {
