...
export function registerDaftraPaymentTools(
	server: McpServer,
	agent: RuntimeAgent,
) {
	if (hasCapability(agent, "daftra.payments.create")) {
		server.registerTool(
			"daftra_add_invoice_payment",
			{
				...
				description:
					"Records a financial payment or receipt against a specific Daftra sales invoice.",
			...
			async (args) => {
				...
				try {
					...
					await logDaftraAction({
						agentId: agent.id,
						action: "daftra_add_invoice_payment",
						entityType: "payment",
						entityId: data.id,
						success: true,
