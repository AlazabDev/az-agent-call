...
const TEMPLATES_DIR = process.env.TEMPLATES_DIR ? path.resolve(process.env.TEMPLATES_DIR) : path.join(process.cwd(), 'templates');
const EXPECTED_TEMPLATE_COUNT = Number(process.env.EXPECTED_TEMPLATE_COUNT ?? 144);
...
const DEFAULT_LOGO_URL = process.env.EMAIL_LOGO_URL ?? 'https://alazab.com/w.gif';
const DEFAULT_WEBSITE_URL = process.env.COMPANY_WEBSITE_URL ?? 'https://alazab.com';
...
function requireString(value: unknown, field: string): string {
  if (typeof value !== 'string' || !value.trim()) {
    throw new Error(`[az-agent-mail] template metadata field "${field}" is missing or invalid.`);
  }
  return value.trim();
}
...
function loadCatalog(): LoadedTemplate[] {
  if (!existsSync(TEMPLATES_DIR)) {
    throw new Error(`[az-agent-mail] templates directory not found: ${TEMPLATES_DIR}`);
  }

  const loaded: LoadedTemplate[] = [];
  const ids = new Set<string>();

  for (const systemDir of readdirSync(TEMPLATES_DIR, {withFileTypes: true}).filter((entry) => entry.isDirectory())) {
    const folder = systemDir.name;
    const dir = path.join(TEMPLATES_DIR, folder);
    const metaFiles = readdirSync(dir).filter((name) => name.endsWith('.meta.json')).sort();
    ...
    for (const metaFile of metaFiles) {
      ...
      if (!existsSync(htmlPath) || !existsSync(textPath)) {
        throw new Error(`[az-agent-mail] template ${folder}/${base} must have .html, .txt and .meta.json files.`);
      }

      const raw = JSON.parse(readFileSync(metaPath, 'utf8')) as Partial<TemplateMetadata>;
      const id = requireString(raw.id, 'id');
      const system = requireString(raw.system, 'system');
      const agent = requireString(raw.agent, 'agent');

      if (ids.has(id)) throw new Error(`[az-agent-mail] duplicate template id: ${id}`);
      if (system !== folder) {
        throw new Error(`[az-agent-mail] template ${id} says system=${system} but lives under templates/${folder}.`);
      }
      if (agent !== `az-agent-${system}`) {
        throw new Error(`[az-agent-mail] template ${id} ownership mismatch: ${agent} does not match its system ${system}.`);
      }
      if (!EXPECTED_AGENT_IDENTITIES.has(agent)) {
        throw new Error(`[az-agent-mail] template ${id} points to unknown agent ${agent}.`);
      }
      if (!VALID_TONES.has(raw.defaultStatusTone as TemplateTone)) {
        throw new Error(`[az-agent-mail] template ${id} has invalid defaultStatusTone.`);
      }
      if (!Array.isArray(raw.required) || !Array.isArray(raw.optional)) {
        throw new Error(`[az-agent-mail] template ${id} must declare required[] and optional[].`);
      }
  ...
  if (Number.isFinite(EXPECTED_TEMPLATE_COUNT) && loaded.length !== EXPECTED_TEMPLATE_COUNT) {
    throw new Error(
      `[az-agent-mail] template catalog mismatch: expected ${EXPECTED_TEMPLATE_COUNT}, found ${loaded.length} in ${TEMPLATES_DIR}.`,
    );
  }
  return loaded;
}
...
export function templateCountsByAgent(): Record<string, number> {
  ...
  for (const template of CATALOG) counts[template.agent] = (counts[template.agent] ?? 0) + 1;
  return counts;
}

export function templatesForAgent(agent: RuntimeAgent): TemplatePublicMetadata[] {
  return CATALOG.filter((template) => template.agent === agent.foundry_id).map(publicMetadata);
}
...
function catalogTemplate(templateId: string): LoadedTemplate {
  ...
  if (!template) {
    throw new EmailTemplateError('القالب غير موجود', 'TEMPLATE_NOT_FOUND', {templateId});
  }
  return template;
}
...
export function templateMetadataForAgent(_agent: RuntimeAgent, templateId: string): TemplatePublicMetadata {
...
function validateTemplateData(template: LoadedTemplate, input: EmailTemplateData): EmailTemplateData {
  ...
  for (const requiredField of template.required) {
    if (input[requiredField] === undefined || input[requiredField] === null || input[requiredField] === '') {
      throw new EmailTemplateError(`الحقل ${requiredField} مطلوب`, 'MISSING_FIELD', {field: requiredField});
    }
  }

  output.recipient_name = boundedString(input.recipient_name, 'recipient_name', 200, true)!;
  ...
  output.secondary_action_label = boundedString(input.secondary_action_label, 'secondary_action_label', 100);
  output.alert_title = boundedString(input.alert_title, 'alert_title', 150);
  output.alert_message = boundedString(input.alert_message, 'alert_message', 1500);
  ...
  if (input.details !== undefined) {
    ...
    output.details = input.details.map((detail, index) => ({
      label: boundedString(detail?.label, `details[${index}].label`, 120, true)!,
      value: boundedString(detail?.value, `details[${index}].value`, 700, true)!,
    }));
  }
...
function renderSections(template: string, data: Record<string, unknown>, html: boolean): string {
  ...
  let output = template.replace(sectionPattern, (_match, key: string, block: string) => {
    ...
    if (Array.isArray(value)) {
      return value.map((item) => renderString(block, {
        ...data,
        ...(item && typeof item === 'object' ? item as Record<string, unknown> : {'.': item}),
      }, html)).join('');
    }
    if (value && typeof value === 'object') {
      return renderString(block, {...data, ...(value as Record<string, unknown>)}, html);
    }
  ...
  output = output.replace(invertedPattern, (_match, key: string, block: string) => {
    const value = getValue(data, key);
    const empty = value === undefined || value === null || value === false || value === '' ||
...
function renderString(template: string, data: Record<string, unknown>, html: boolean): string {
  let output = template;
  let previous = '';
  let iterations = 0;
...
export function renderTemplateForAgent(
  ...
  const merged: Record<string, unknown> = {
    ...
    footer_note: data.footer_note ?? 'هذه رسالة تشغيلية آلية صادرة من نظام العزب.',
  ...
  const subject = renderString(template.subject, merged, false).replace(/[\r\n]+/g, ' ').trim();
  const html = renderString(template.html, merged, true);
  const text = renderString(template.text, merged, false).replace(/\n{3,}/g, '\n\n').trim() + '\n';

  if (/{{[\s\S]*?}}/.test(subject) || /{{[\s\S]*?}}/.test(html) || /{{[\s\S]*?}}/.test(text)) {
    throw new EmailTemplateError('تعذر حل جميع متغيرات القالب', 'UNRESOLVED_TEMPLATE_VARIABLE', {templateId});
  }

  return {templateId, subject, html, text, metadata: publicMetadata(template)};
}
