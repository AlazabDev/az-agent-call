...
function requireString(value: unknown, field: string): string {
  if (typeof value !== 'string' || !value.trim()) {
    throw new Error(`[az-agent-call] template metadata field "${field}" is missing or invalid.`);
  }
  return value.trim();
}
...
function loadCatalog(): LoadedTemplate[] {
  if (!existsSync(TEMPLATES_DIR)) {
    throw new Error(`[az-agent-call] templates directory not found: ${TEMPLATES_DIR}`);
  }
  ...
  for (const systemDir of readdirSync(TEMPLATES_DIR, {withFileTypes: true}).filter((entry) => entry.isDirectory())) {
    ...
    for (const metaFile of metaFiles) {
      ...
      if (!existsSync(htmlPath) || !existsSync(textPath)) {
        throw new Error(`[az-agent-call] template ${folder}/${base} must have .html, .txt and .meta.json files.`);
      }
      ...
      if (ids.has(id)) throw new Error(`[az-agent-call] duplicate template id: ${id}`);
      if (system !== folder) {
        throw new Error(`[az-agent-call] template ${id} says system=${system} but lives under templates/${folder}.`);
      }
      if (agent !== `az-agent-${system}`) {
        throw new Error(`[az-agent-call] template ${id} ownership mismatch: ${agent} does not match its system ${system}.`);
      }
      if (!EXPECTED_AGENT_IDENTITIES.has(agent)) {
        throw new Error(`[az-agent-call] template ${id} points to unknown agent ${agent}.`);
      }
      if (!VALID_TONES.has(raw.defaultStatusTone as TemplateTone)) {
        throw new Error(`[az-agent-call] template ${id} has invalid defaultStatusTone.`);
      }
      if (!Array.isArray(raw.required) || !Array.isArray(raw.optional)) {
        throw new Error(`[az-agent-call] template ${id} must declare required[] and optional[].`);
      }
