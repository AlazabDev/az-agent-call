...
export async function assertSupabaseProjectBinding(): Promise<void> {
  ...
  const databaseRef = typeof value.supabase_project_ref === "string" ? value.supabase_project_ref : "";

  if (!databaseRef) {
    throw new Error("Supabase runtime setting is missing supabase_project_ref");
  }
  if (databaseRef !== config.supabaseProjectRef) {
    throw new Error(`Supabase project mismatch: app=${config.supabaseProjectRef}, database=${databaseRef}`);
  }
}
