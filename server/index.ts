...
app.use(express.json({
  limit: "512kb",
  // Keep the raw request bytes around so the WhatsApp webhook handler can
  // verify Meta's X-Hub-Signature-256 HMAC before trusting the parsed body.
  verify: (req, _res, buf) => {
...
app.get("/readyz", async (_req, res) => {
  try {
    const [agents, templates] = await Promise.all([
      supabaseAdmin.from("call_agents").select("id, enabled"),
      supabaseAdmin.from("call_templates").select("id", { count: "exact", head: true }).eq("enabled", true),
    ]);
