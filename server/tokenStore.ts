...
async function syncHashes(generated: ReadonlySet<AgentId>): Promise<void> {
	for (const id of AGENT_IDS) {
		...
		const { error } = await supabaseAdmin
			...
			.eq("id", id);
		if (error)
			throw new Error(`Failed to sync token hash for ${id}: ${error.message}`);
	}
}

export async function initializeAgentTokens(): Promise<{
	generated: AgentId[];
	path: string;
}> {
	try {
		const parsed = JSON.parse(
			await readFile(config.agentTokensPath, "utf8"),
...
export async function rotateStoredAgentToken(
	id: AgentId,
