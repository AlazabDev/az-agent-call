...
import { INITIAL_WHATSAPP_NUMBERS, type WhatsAppNumberRecord } from "./config.js";
...
export class WhatsAppService {
  ...
  public async getNumberForAgent(agentId: string): Promise<WhatsAppNumberRecord | null> {
    const numbers = await this.getNumbers();
    const assigned = numbers.find((n) => n.allocatedAgentId === agentId && n.isActive);
    if (assigned) return assigned;
    return numbers.find((n) => n.role === "agent" && n.isActive) || numbers[0] || null;
  }
  ...
  public async sendTextMessage(options: {
    ...
    if (options.phoneNumberId) {
      const numbers = await this.getNumbers();
      senderNumberRecord = numbers.find((n) => n.phoneNumberId === options.phoneNumberId) || null;
    ...
    if (!senderNumberRecord) {
      return { ok: false, error: "No active WhatsApp sender number available." };
    }
