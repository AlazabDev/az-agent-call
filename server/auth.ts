...
async function resolveMailRole(userId: string): Promise<{ role: MailAdminRole; source: "adp_user_roles" | "mail_admins" } | null> {
...
export async function requireAdmin(req: AdminRequest, res: Response, next: NextFunction) {
  ...
  try {
    ...
    if (!resolved) return res.status(403).json({ error: "Az Agent Mail access required" });
