...
export class SmartResolver {
	public async searchEntities(
		...
		const fetchAndMatch = async (
			...
			try {
				...
				try {
					...
					items = Array.isArray(resV1?.data)
						? resV1.data
						: Array.isArray(resV1)
							? resV1
							: [];
				} catch {
					const resV2 = await daftraClient.get(
						`/v2/api/entity/${v2Key}/list/1`,
					);
					items = Array.isArray(resV2?.data)
						? resV2.data
						: Array.isArray(resV2)
							? resV2
							: [];
				...
				for (const rawItem of items) {
					...
					let confidence = 0;
					let matchType: "id" | "exact" | "alias" | "normalized" | "fuzzy" =
						"normalized";
		...
		if (entityType === "all" || entityType === "client") {
			tasks.push(
				fetchAndMatch("/api2/clients.json", "client", [
					"first_name",
					"last_name",
					"business_name",
					"email",
		...
		if (entityType === "all" || entityType === "supplier") {
			tasks.push(
				fetchAndMatch("/api2/suppliers.json", "supplier", [
					"first_name",
					"last_name",
					"business_name",
					"email",
		...
		if (entityType === "all" || entityType === "work_order") {
			tasks.push(
				fetchAndMatch("/api2/work_orders.json", "work_order", [
					"name",
					"title",
					"order_no",
				]),
		...
		if (entityType === "all" || entityType === "product") {
			tasks.push(
				fetchAndMatch("/api2/products.json", "product", [
					"name",
					"product_code",
					"description",
				]),
	...
	public async resolveSingleEntity(
		...
		if (
			matches.length === 1 ||
			(matches[0].confidence >= 0.95 && matches[1]?.confidence < 0.8)
		) {
			const top = matches[0];
			return {
				status: "resolved",
