...
export const daftraClientSearchSchema = z.object({
  query: z.string().optional().describe("Search term, name, email, or client ID"),
...
export const daftraSearchEntitiesSchema = z.object({
  query: z.string().min(1).describe("Name, SKU, phone, or keyword to search e.g. 'عوف', 'اربيسك', 'رخام'"),
  entity_type: z.enum(["all", "client", "supplier", "work_order", "product", "cost_center", "treasury"]).optional().default("all"),
});
