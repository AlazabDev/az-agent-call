    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401 || response.status === 403) {
          throw new DaftraError("DAFTRA_AUTH_FAILED", "Daftra API authentication failed", response.status, data);
        }
        if (response.status === 404) {
          throw new DaftraError("DAFTRA_NOT_FOUND", `Resource not found on Daftra ERP: ${path}`, response.status, data);
        }
        throw new DaftraError(
          "DAFTRA_UNAVAILABLE",
      ...
      if (err.name === "AbortError") {
        throw new DaftraError("DAFTRA_TIMEOUT", `Daftra API request timed out after ${timeout}ms: ${method} ${path}`);
      }

      throw new DaftraError("DAFTRA_UNAVAILABLE", `Daftra network request failed: ${err.message || String(err)}`);
    }
  }
