...
export async function listInvoices(
	params: { client_id?: number; status?: string; limit?: number } = {},
...
export async function createInvoice(data: {
	...
	const formattedItems = data.items.map((it) => ({
		item: it.name || "Service / Line Item",
	...
	if (!created?.id) {
		throw new DaftraError(
			...
			"Failed to confirm invoice creation response from Daftra ERP",
		);
	}
...
export async function createSmartInvoice(data: {
	...
	const clientRes = await smartResolver.resolveSingleEntity(
		data.client_query,
		"client",
	...
	if (clientRes.status === "not_found") {
		throw new DaftraError(
			"ENTITY_NOT_FOUND",
			`Client '${data.client_query}' not found in Daftra ERP`,
		);
	}
	if (clientRes.status === "ambiguous") {
		throw new DaftraError(
			"AMBIGUOUS_ENTITY",
			`Client name '${data.client_query}' is ambiguous`,
			400,
			{
	...
	for (const rawIt of data.items) {
		...
		if (rawIt.product_query) {
			const prodRes = await smartResolver.resolveSingleEntity(
				rawIt.product_query,
				"product",
				["name", "product_code"],
			);
		...
		if (!price || price <= 0) {
			throw new DaftraError(
				...
				`Unit price required for item '${itemName}'`,
			);
		}
