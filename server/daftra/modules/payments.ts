...
export async function addInvoicePayment(data: {
	...
	if (
		...
		throw new DaftraError(
			...
			`Invoice #${data.invoice_id} is already fully paid.`,
		);
	}
	...
	const response = await daftraClient.post(
		"/api2/invoice_payments.json",
		payload,
	);
	const created =
		response?.data?.InvoicePayment ||
	...
	if (!created?.id) {
		throw new DaftraError(
			...
			"Failed to confirm payment creation response from Daftra ERP",
		);
	}
