...
): Promise<DaftraProductRecord[]> {
	const response = await daftraClient.get("/api2/products.json", {
	...
	let products: DaftraProductRecord[] = rawList.map((item: any) => {
		...
		return {
			...
			category_id: p.category_id
				? parseInt(String(p.category_id), 10)
				: undefined,
		};
	});
...
export async function getProduct(
	...
	if (typeof idOrQuery === "number" || /^\d+$/.test(String(idOrQuery).trim())) {
		...
		if (!p?.id) {
			throw new DaftraError(
				"DAFTRA_NOT_FOUND",
				`Product #${productId} not found on Daftra ERP`,
				404,
			);
		}
		return {
	...
	if (matches.length === 0) {
		throw new DaftraError(
			"ENTITY_NOT_FOUND",
			`Product matching '${idOrQuery}' not found`,
			404,
		);
	}
	if (matches.length > 1) {
		throw new DaftraError(
			"AMBIGUOUS_ENTITY",
			`Multiple products match '${idOrQuery}'`,
			400,
			{ matches },
		);
	}
...
export async function createProduct(
	data: Partial<DaftraProductRecord>,
): Promise<DaftraProductRecord> {
	const payload = {
		Product: {
			name: data.name,
