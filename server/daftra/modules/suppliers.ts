...
export async function listSuppliers(params: { query?: string; limit?: number } = {}): Promise<DaftraSupplierRecord[]> {
  const response = await daftraClient.get("/api2/suppliers.json", { limit: params.limit || 50 });
...
export async function getSupplier(idOrQuery: number | string): Promise<DaftraSupplierRecord> {
  if (typeof idOrQuery === "number" || /^\d+$/.test(String(idOrQuery).trim())) {
    const supplierId = parseInt(String(idOrQuery).trim(), 10);
    const response = await daftraClient.get(`/api2/suppliers/${supplierId}.json`);
    const s = response?.data?.Supplier || response?.data || response;
    if (!s || !s.id) {
      throw new DaftraError("DAFTRA_NOT_FOUND", `Supplier #${supplierId} not found on Daftra ERP`, 404);
    }
    return {
      id: parseInt(String(s.id), 10),
  ...
  if (matches.length === 0) {
    throw new DaftraError("ENTITY_NOT_FOUND", `Supplier matching '${idOrQuery}' not found`, 404);
  }
  if (matches.length > 1) {
    throw new DaftraError("AMBIGUOUS_ENTITY", `Multiple suppliers match '${idOrQuery}'`, 400, { matches });
  }
  return matches[0];
}

export async function createSupplier(data: Partial<DaftraSupplierRecord>): Promise<DaftraSupplierRecord> {
  const payload = {
    Supplier: {
