...
import type { CustomerCallerContext, DaftraClientRecord, DaftraInvoiceRecord, DaftraWorkOrderRecord } from "../types.js";

export async function listClients(params: { query?: string; phone?: string; limit?: number } = {}): Promise<DaftraClientRecord[]> {
  const response = await daftraClient.get("/api2/clients.json", { limit: params.limit || 50 });
  ...
  if (params.phone) {
    ...
    clients = clients.filter(
      (c) => normalizePhone(c.phone1).includes(normSearchPhone) || normalizePhone(c.phone2).includes(normSearchPhone)
    );
  }
...
export async function getClient(idOrQuery: number | string): Promise<DaftraClientRecord> {
  if (typeof idOrQuery === "number" || /^\d+$/.test(String(idOrQuery).trim())) {
    ...
    if (!data || !data.id) {
      throw new DaftraError("DAFTRA_NOT_FOUND", `Client #${clientId} not found on Daftra ERP`, 404);
    }
    return {
      id: parseInt(String(data.id), 10),
  ...
  if (matches.length === 0) {
    throw new DaftraError("ENTITY_NOT_FOUND", `Client matching '${idOrQuery}' not found`, 404);
  }
  if (matches.length > 1) {
    throw new DaftraError("AMBIGUOUS_ENTITY", `Multiple clients match '${idOrQuery}'`, 400, { matches });
  }
  return matches[0];
}

export async function createClient(data: Partial<DaftraClientRecord>): Promise<DaftraClientRecord> {
  const payload = {
    Client: {
      first_name: data.first_name || "",
      last_name: data.last_name || "",
 ...
 */
export async function lookupCallerContext(phoneNumber: string): Promise<CustomerCallerContext> {
  ...
  try {
    const invRes = await daftraClient.get("/api2/invoices.json", { client_id: client.id, limit: 20 });
    const rawInvoices = Array.isArray(invRes?.data) ? invRes.data : Array.isArray(invRes) ? invRes : [];
    ...
    openInvoices = recentInvoices.filter((inv) => inv.status !== "paid" && (inv.balance || 0) > 0);
  } catch {
    // Non-fatal if invoices fail
  }

  try {
    const woRes = await daftraClient.get("/v2/api/entity/work_order/list/1", { client_id: client.id });
    const rawWo = Array.isArray(woRes?.data) ? woRes.data : Array.isArray(woRes) ? woRes : [];
  ...
  const totalBilled = recentInvoices.reduce((sum, inv) => sum + (inv.total || 0), 0);
  const paid = recentInvoices.reduce((sum, inv) => sum + (inv.paid || 0), 0);
  const unpaid = recentInvoices.reduce((sum, inv) => sum + (inv.balance || 0), 0);
  ...
  return {
    ...
    balance: {
      ...
      paid,
      unpaid,
      overdue: openInvoices.filter((inv) => inv.status === "overdue").reduce((sum, inv) => sum + (inv.balance || 0), 0),
    },
    matchedBy: "phone",
  };
}
