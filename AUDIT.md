# Az Agent Mail v3.0.1 — Final Production Gate
...
- Page files: **18**; the previously abbreviated operational pages were rebuilt for Agent Mail rather than Daftra.
...
- Mailer forces `From` and `Reply-To` to the bearer-token Agent mailbox server-side.
...
- Persistent token data: `/var/lib/az-agent-mail/data:/app/data`.
...
- Production archive layout is root-flat so it can be extracted directly into `/var/www/apps/az-agent-mail` without an extra nested directory.

## Supabase production verification

Direct checks against `alazab-db` (`bxuhcbfdoaflsgbxiqei`) confirmed:

- Agent Mail migrations present:
...
- `mail_agent_connections`: **12** rows.
- `sso_apps.agent-mail`: active and points to `https://mcp.alazab.com/admin/`.
- `mail_settings.runtime.supabase_project_ref`: correct production ref.
- `mail_settings.template_access`: `scope=global`, `catalog_size=144`, `ownership=recommended_only`.
- Edge Function `agent-mail-status`: **ACTIVE**, **version 3**, `verify_jwt=true`.
- Central access currently includes one `platform_owner`.
- RLS SELECT policies exist for all Agent Mail public tables using `mail_private.is_authorized()`.
...
- enabled `mail_templates`: **0** — expected; startup syncs the Git catalog to 144.
...
- `mail_gateway_instances`: **0** — expected; first Gateway heartbeat creates the runtime row.
