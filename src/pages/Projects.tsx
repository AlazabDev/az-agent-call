  ...
  return <AppLayout title="بوابة التشغيل" subtitle="مركز نشر وتشغيل Gateway على Docker/Nginx وربطه بـalazab-db"
    ...
    <div className="space-y-5">
      ...
      <Tabs defaultValue="instances">
        ...
        <TabsContent value="instances"><Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><Server className="h-4 w-4" /> Gateway Instances</CardTitle><CardDescription>حالة Runtime الفعلية من `mail_gateway_instances`.</CardDescription></CardHeader><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>Instance</TableHead><TableHead>Host</TableHead><TableHead>Version</TableHead><TableHead>Status</TableHead><TableHead>SMTP</TableHead><TableHead>Templates</TableHead><TableHead>Started</TableHead><TableHead>Last Seen</TableHead></TableRow></TableHeader><TableBody>{gateways.data?.map((g) => <TableRow key={g.instanceId}><TableCell className="font-mono text-xs" dir="ltr">{g.instanceId}</TableCell><TableCell dir="ltr">{g.hostname}</TableCell><TableCell>{g.version}</TableCell><TableCell><Badge variant={g.status === "ready" ? "default" : "outline"}>{g.status}</Badge></TableCell><TableCell className="font-mono text-xs" dir="ltr">{g.smtpHost}:{g.smtpPort}</TableCell><TableCell>{g.templateCount}</TableCell><TableCell className="text-xs">{fmt(g.startedAt)}</TableCell><TableCell className="text-xs">{fmt(g.lastSeenAt)}</TableCell></TableRow>)}</TableBody></Table>{!gateways.data?.length && <p className="p-8 text-center text-sm text-muted-foreground">لا توجد Instances مسجلة حتى الآن.</p>}</CardContent></Card></TabsContent>
        ...
        <TabsContent value="docker"><div className="grid xl:grid-cols-2 gap-4"><Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><Container className="h-4 w-4" /> Container Hardening</CardTitle></CardHeader><CardContent className="space-y-2 text-sm">{["Node 22 Alpine multi-stage", "read_only root filesystem", "tmpfs /tmp", "cap_drop: ALL", "no-new-privileges", "bind 127.0.0.1:3300", "persistent /app/data", "healthcheck enabled"].map((x) => <div key={x} className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-600" /> {x}</div>)}</CardContent></Card><Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><HardDrive className="h-4 w-4" /> Persistence</CardTitle></CardHeader><CardContent className="space-y-4 text-sm"><div><p className="text-muted-foreground">Host</p><code dir="ltr">/var/lib/az-agent-mail/data</code></div><Separator /><div><p className="text-muted-foreground">Container</p><code dir="ltr">/app/data</code></div><Separator /><div><p className="text-muted-foreground">Token file</p><code dir="ltr">/app/data/agent-tokens.json</code></div></CardContent></Card></div></TabsContent>
