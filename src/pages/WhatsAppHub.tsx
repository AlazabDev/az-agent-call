...
export default function WhatsAppHub() {
	...
	const numbersQuery = useQuery({
		...
		queryFn: async () => {
			const res = await apiFetch<{ ok: boolean; data: WhatsAppNumberRecord[] }>(
			...
			<div className="space-y-6" dir="rtl">
				<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
					...
					<Card className="shadow-card">
						<CardContent className="p-4">
							<p className="text-xs text-muted-foreground">
								أرقام وكلاء الاتصال (Agents)
							</p>
							<p className="text-2xl font-bold mt-1">
								{agentLines.length} أرقام
							</p>
							<p className="text-[10px] text-muted-foreground mt-2">
								تكامل مباشر مع MCP
							</p>
					...
					<Card className="shadow-card">
						<CardContent className="p-4">
							<p className="text-xs text-muted-foreground">
								أرقام الفريق والمشاريع
							</p>
							<p className="text-2xl font-bold mt-1">
								{teamLines.length + projectLines.length} أرقام
							</p>
							<p className="text-[10px] text-muted-foreground mt-2">
								توجيه العمليات والمشروعات
							</p>
						</CardContent>
					</Card>
				</div>
				...
				<Card className="shadow-card">
					<CardHeader>
						<CardTitle className="text-base flex items-center gap-2">
							<Smartphone className="h-4 w-4 text-primary" /> دليل أرقام الوتساب
							والـ WABAs الموزعة
						</CardTitle>
						<CardDescription>
							خريطة توزيع خطوط الوتساب الـ 9 على وكلاء الذكاء الاصطناعي وفريق
							العمليات
						</CardDescription>
					</CardHeader>
				...
				<Card className="shadow-card overflow-hidden">
					<CardHeader>
						<CardTitle className="text-base flex items-center gap-2">
							<MessageSquare className="h-4 w-4 text-primary" /> سجل إشعارات
							ورسائل الوتساب الحية
						</CardTitle>
						<CardDescription>
							الرسائل المباشرة والقوالب الصادرة والواردة عبر كافة الأرقام
						</CardDescription>
					</CardHeader>
					<CardContent className="p-0">
						<Table>
								...
								{logs.map((log) => (
									<TableRow key={log.id}>
										...
										<TableCell>
											<Badge
												variant={
													log.direction === "outbound" ? "default" : "secondary"
												}
											>
