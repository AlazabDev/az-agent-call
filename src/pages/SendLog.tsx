...
export default function SendLog() {
		...
		() =>
			(logs.data ?? []).filter((row) => {
				...
				return (
					...
					(!q ||
						`${row.recipient} ${row.subject} ${row.messageId ?? ""}`
							.toLowerCase()
							.includes(q))
				);
			}),
	...
	return (
		<AppLayout
			title="سجل الإرسال"
			subtitle="Audit مركزي لكل رسالة خرجت من Migadu عبر الوكلاء أو لوحة الإدارة"
		>
			<div className="space-y-5">
				<Card>
					<CardHeader>
						<CardTitle className="text-base flex items-center gap-2">
							<InboxIcon className="h-4 w-4" /> Mail Send Log
						</CardTitle>
						<CardDescription>
							المرسل ثابت ومحفوظ مع كل عملية، حتى لو تغير إعداد الوكيل لاحقًا.
						</CardDescription>
					</CardHeader>
					<CardContent>
						<div className="grid md:grid-cols-[1fr_180px_160px] gap-3">
							<div className="relative">
								...
								<Input
									...
									placeholder="المستلم، الموضوع، Message ID"
								/>
							</div>
							...
							<TableBody>
								{rows.map((row) => (
									<TableRow key={row.id}>
										...
										<TableCell dir="ltr" className="font-mono text-[11px]">
											{row.senderMailbox}
										</TableCell>
										<TableCell
											dir="ltr"
										...
										<TableCell dir="ltr" className="text-[11px]">
