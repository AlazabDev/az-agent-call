...
const AGENT_SPECIALTIES: Record<AgentId, AgentSpec> = {
  backend: { role: "البنية التحتية للخوادم", desc: "إدارة الخوادم، الاتصالات، وبوابات API", color: "#3B82F6", icon: "💻" },
  azabot: { role: "المساعد المباشر العام", desc: "الإجابة التفاعلية والبريد الذكي", color: "#FFB900", icon: "🤖" },
  auth: { role: "الأمان والمصادقة", desc: "إدارة الرموز وصلاحيات الوصول", color: "#EF4444", icon: "🔐" },
  prod: { role: "إدارة خطوط الإنتاج", desc: "متابعة أوامر التشغيل والمخرجات", color: "#10B981", icon: "🏭" },
  maint: { role: "الصيانة والتشغيل", desc: "متابعة البلاغات وبلاغات الأعطال", color: "#F59E0B", icon: "🔧" },
  core: { role: "المركزي وتنسيق النظام", desc: "تنسيق 144 قالب وإدارة الوكلاء", color: "#6366F1", icon: "⚡" },
  bim: { role: "النمذجة الهندسية BIM", desc: "معالجة وتحليل مخططات البناء", color: "#8B5CF6", icon: "🏗️" },
  finance: { role: "المالية والمحاسبة", desc: "الفواتير والحسابات والتأكيدات", color: "#06B6D4", icon: "💰" },
  payments: { role: "التحصيلات والمدفوعات", desc: "معالجة وتأكيد سندات القبض", color: "#EC4899", icon: "💳" },
  copilot: { role: "المساعد البرمجي", desc: "مراجعة وحل الأكواد البرمجية", color: "#14B8A6", icon: "🚀" },
  project: { role: "إدارة المشاريع", desc: "تتبع المهام والمخططات الزمنية", color: "#3B82F6", icon: "📊" },
  vision: { role: "الرؤية والذكاء الاصطناعي", desc: "تحليل الصور والمخططات والوثائق", color: "#F43F5E", icon: "👁️" },
};
...
export default function AgentChat() {
  ...
  const activeSession = sessions.find((s: ChatSession) => s.id === currentSessionId);
  ...
  const handleNewChat = () => {
    const newSess = createNewSession(selectedAgentId, `جلسة جديدة - ${AGENT_LABELS[selectedAgentId]}`);
    const updated = [newSess, ...sessions];
    setSessions(updated);
    saveChatSessions(updated);
    setCurrentSessionId(newSess.id);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (file.type.includes("text") || file.type.includes("json") || file.name.endsWith(".ts") || file.name.endsWith(".js") || file.name.endsWith(".md")) {
        try {
    ...
    setAttachments((prev: FileFeedAttachment[]) => [...prev, ...newAttachments]);
    toast.success(`تم إضافة ${newAttachments.length} ملف لتغذية الوكيل بها.`);
    if (fileInputRef.current) fileInputRef.current.value = "";
  };

  const removeAttachment = (id: string) => {
    setAttachments((prev: FileFeedAttachment[]) => prev.filter((a: FileFeedAttachment) => a.id !== id));
  };
  ...
  const handleSendMessage = async () => {
    ...
    const userMsg: ChatMessage = {
      ...
      role: "user",
      content: userMessageContent || (userAttachments.length > 0 ? "[تم إرفاق ملفات تغذية]" : ""),
    ...
    setTimeout(() => {
      const responseText = generateAgentResponse(selectedAgentId, userMessageContent, userAttachments);
  ...
  const confirmDeleteSession = () => {
    ...
    const remaining = sessions.filter((s: ChatSession) => s.id !== sessionToDelete);
  ...
  const agentSpec: AgentSpec = AGENT_SPECIALTIES[selectedAgentId] || AGENT_SPECIALTIES.azabot;
    ...
    <AppLayout title="دردشة وتفاعل الوكلاء (Agent Chat & Sessions)">
      <div className="h-[calc(100vh-8.5rem)] flex gap-4 overflow-hidden">
        ...
        <Card className="w-80 flex flex-col border-border bg-card shadow-lg shrink-0 overflow-hidden">
          <CardHeader className="p-3 border-b border-border bg-muted/30">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-sm font-bold">الجلسات والمحادثات</CardTitle>
              </div>
              <Button onClick={handleNewChat} size="sm" className="h-8 bg-[#030957] hover:bg-[#071280] text-white text-xs gap-1 font-bold">
          ...
          <div className="p-3 border-b border-border bg-muted/10 space-y-1.5">
            <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider block">تبديل الوكيل الفعّال</label>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="w-full justify-between h-11 bg-background border-border hover:bg-muted/50 text-right px-3">
                  <div className="flex items-center gap-2 truncate">
                    <span className="text-lg">{agentSpec.icon}</span>
                    <div className="truncate">
                      <div className="text-xs font-bold text-foreground truncate">{AGENT_LABELS[selectedAgentId]}</div>
                      <div className="text-[10px] text-muted-foreground truncate">{foundryId(selectedAgentId)}</div>
                    </div>
                  </div>
                  ...
                  return (
                    <DropdownMenuItem
                      ...
                      className={`flex items-center justify-between p-2.5 rounded-md cursor-pointer ${
                        isSelected ? "bg-[#030957] text-white font-bold" : "hover:bg-muted"
                      }`}
                    >
                      <div className="flex items-center gap-2.5 truncate">
                        <span className="text-base">{spec.icon}</span>
                        <div className="truncate">
                          <div className="text-xs font-bold">{AGENT_LABELS[id]}</div>
                          <div className={`text-[10px] ${isSelected ? "text-slate-300" : "text-muted-foreground"}`}>{spec.role}</div>
                        </div>
                      </div>
                      {isSelected && <Check className="h-4 w-4 text-[#FFB900]" />}
                    </DropdownMenuItem>
                  );
                })}
              </DropdownMenuContent>
          ...
          <ScrollArea className="flex-1 p-2">
            <div className="space-y-1">
              {sessions.map((sess: ChatSession) => {
                ...
                const spec = AGENT_SPECIALTIES[sess.agentId] || AGENT_SPECIALTIES.azabot;
                return (
                  <div
                    key={sess.id}
                    onClick={() => {
                  ...
                  >
                    <div className="flex items-center gap-2.5 truncate min-w-0">
                      ...
                      <div className="truncate min-w-0">
                        <div className={`text-xs truncate ${isActive ? "font-bold text-foreground" : "font-medium"}`}>
                          {sess.title}
                        </div>
                        <div className="text-[10px] text-muted-foreground flex items-center gap-1.5 mt-0.5">
                          <span>{sess.messages.length} رسالة</span>
                          <span>•</span>
                          <span>{new Date(sess.updatedAt).toLocaleTimeString("ar-SA", { hour: "2-digit", minute: "2-digit" })}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={(e: React.MouseEvent) => handleDeleteSession(sess.id, e)}
        ...
        <Card className="flex-1 flex flex-col border-border bg-card shadow-xl overflow-hidden relative">
          <CardHeader className="p-3 border-b border-border bg-gradient-to-r from-[#030957] to-[#071280] text-white flex-row items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-base font-bold text-white">{AGENT_LABELS[selectedAgentId]}</h3>
                  <Badge className="bg-[#FFB900] text-[#030957] font-bold text-[10px]">{agentSpec.role}</Badge>
              ...
              return (
                <div key={msg.id} className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
                  ...
                  <div className={`max-w-[75%] space-y-2 ${isUser ? "items-end" : "items-start"}`}>
                        ...
                        <div className="mt-2.5 pt-2 border-t border-white/20 space-y-1.5">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {msg.attachments.map((att: FileFeedAttachment) => (
                              <div key={att.id} className="flex items-center gap-1.5 px-2 py-1 bg-black/20 rounded text-xs">
                                <FileText className="h-3.5 w-3.5 text-[#FFB900]" />
                                <span className="font-mono text-[11px] truncate max-w-[140px]">{att.name}</span>
                                <span className="opacity-70 text-[10px]">({formatFileSize(att.size)})</span>
                              </div>
                            ))}
                          </div>
                        </div>
                    ...
                    <div className={`text-[10px] text-muted-foreground px-1 ${isUser ? "text-left" : "text-right"}`}>
            ...
            {isSending && (
              <div className="flex gap-3 justify-start items-center">
                ...
                <div className="p-3 bg-background border border-border rounded-2xl rounded-tr-none flex items-center gap-2 shadow">
                  <span className="text-xs text-muted-foreground font-medium">جاري معالجة الطلب وتغذية الوكيل...</span>
          ...
          {attachments.length > 0 && (
            <div className="p-2 border-t border-border bg-muted/20 flex flex-wrap gap-2">
              {attachments.map((att: FileFeedAttachment) => (
                <div key={att.id} className="flex items-center gap-2 px-2.5 py-1 bg-card border rounded-lg text-xs shadow-sm">
                  <FileCode className="h-4 w-4 text-[#FFB900]" />
                  <span className="font-mono text-xs font-semibold max-w-[160px] truncate">{att.name}</span>
                  <span className="text-muted-foreground text-[10px]">({formatFileSize(att.size)})</span>
                  <button onClick={() => removeAttachment(att.id)} className="text-red-400 hover:text-red-600 p-0.5">
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle className="text-base font-bold text-red-600">حذف جلسة المحادثة</DialogTitle>
            <DialogDescription>هل أنت تأكد من رغبتك في حذف هذه الجلسة؟ لن تتمكن من استعادتها لاحقاً.</DialogDescription>
          </DialogHeader>
...
function generateAgentResponse(agentId: AgentId, userText: string, attachments: FileFeedAttachment[]): string {
  const fileContextNotice =
    attachments.length > 0 ? `\n\n📄 **تمت معالجة التغذية المرفقة (${attachments.length} ملفات):**\n` + attachments.map((a: FileFeedAttachment) => `- ${a.name} (${formatFileSize(a.size)})`).join("\n") : "";
  ...
  return responses[agentId] || `تم استلام طلبك وبدء المعالجة بواسطة الوكيل ${agentId}.${fileContextNotice}`;
}
