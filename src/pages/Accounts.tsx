...
export default function Accounts() {
	...
	const [testBody, setTestBody] = useState(
		"اختبار اتصال SMTP من لوحة Az Agent Call Center.",
	);
	...
	const filtered = useMemo(
		() =>
			agents.filter((agent) => {
				...
				const textMatch =
					!term ||
					[agent.id, agent.mailbox, agent.foundryId].some((value) =>
				...
				const passwordMatch =
					passwordFilter === "all" ||
					agent.smtpPasswordSource === passwordFilter;
	...
	const stats = useMemo(
		() => ({
			...
			override: agents.filter(
				(agent) => agent.smtpPasswordSource === "override",
			).length,
			pattern: agents.filter((agent) => agent.smtpPasswordSource === "pattern")
				.length,
	...
	const rotateMutation = useMutation({
		mutationFn: (agentId: AgentId) =>
			apiFetch<RotateResult>(`/api/agents/${agentId}/rotate-token`, {
				method: "POST",
				body: "{}",
			}),
		onSuccess: (data) => {
		...
		>
			<div className="space-y-6" dir="rtl">
				<div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
					<Card className="shadow-card">
						<CardContent className="p-4">
							<p className="text-xs text-muted-foreground">SIP Extensions</p>
							<p className="text-2xl font-bold mt-1">{agents.length || 12}</p>
							<Progress value={(agents.length / 12) * 100} className="mt-3" />
						</CardContent>
					</Card>
					<Card className="shadow-card">
						<CardContent className="p-4">
							<p className="text-xs text-muted-foreground">Telephony Lines</p>
							<p className="text-2xl font-bold mt-1">{stats.configured}/12</p>
					...
					<Card className="shadow-card">
						<CardContent className="p-4">
							<p className="text-xs text-muted-foreground">Daftra ERP Status</p>
				...
				<Alert>
					...
					<AlertDescription>
						كل Bearer Token يحدد خط وكيل واحد ويدير هوية إجراء واستلام المكالمات
						والعمليات المحاسبية في دفتره بصورة آمنة ومقفولة سيرفرًا.
					</AlertDescription>
				</Alert>

				<Card className="shadow-card">
					<CardHeader>
						<CardTitle className="text-base">فلترة الصناديق</CardTitle>
						<CardDescription>
							ابحث في الهوية أو البريد أو Foundry ID، ثم افتح الصندوق لفحص كل
							تفاصيله واختبار SMTP.
						</CardDescription>
					</CardHeader>
					<CardContent className="grid md:grid-cols-3 gap-3">
						...
						<Select value={passwordFilter} onValueChange={setPasswordFilter}>
							...
							<SelectContent>
								<SelectItem value="all">كل مصادر كلمات المرور</SelectItem>
				...
				<Card className="shadow-card overflow-hidden">
					<CardHeader>
						<CardTitle className="text-base">Mailbox Registry</CardTitle>
						<CardDescription>
							المصدر الفعلي: `public.mail_agents` + حالة الاتصال من
						...
						<Table>
							<TableHeader>
								<TableRow>
									...
									<TableHead>Foundry ID</TableHead>
								...
								{filtered.map((agent) => (
									<TableRow key={agent.id}>
										...
										<TableCell>
											...
											>
												{agent.smtpConfigured ? "Ready" : "Missing"}
											</Badge>
										</TableCell>
										<TableCell>
				...
				<div className="grid lg:grid-cols-3 gap-5">
					<Card className="shadow-card">
						<CardHeader>
							<CardTitle className="text-base flex items-center gap-2">
								<ServerCog className="h-4 w-4" /> SMTP Strategy
							</CardTitle>
						</CardHeader>
						<CardContent className="space-y-3">
							<div className="rounded-lg border p-3">
								...
								<p className="font-mono text-sm mt-1" dir="ltr">
									{settingsQuery.data?.smtpHost ?? "smtp.migadu.com"}:
									{settingsQuery.data?.smtpPort ?? 587}
								</p>
							</div>
							<div className="rounded-lg border p-3">
								...
								<p className="text-[10px] text-muted-foreground mt-1">
									Per-agent override remains the preferred source.
								</p>
							</div>
							<div className="rounded-lg border p-3">
								<p className="text-xs text-muted-foreground">
					...
					<Card className="shadow-card">
						<CardHeader>
							<CardTitle className="text-base flex items-center gap-2">
								<KeyRound className="h-4 w-4" /> Token Store
							</CardTitle>
						</CardHeader>
						<CardContent className="space-y-3">
							<p className="text-xs text-muted-foreground">
								Clear tokens never go to Supabase.
							</p>
							<div
								className="rounded-lg bg-slate-950 text-slate-100 p-3 font-mono text-xs break-all"
								dir="ltr"
							>
								{settingsQuery.data?.agentTokensPath ??
									"/app/data/agent-tokens.json"}
							</div>
							<div className="flex items-center justify-between">
								<span className="text-sm">Loaded</span>
								<Badge>
									{settingsQuery.data?.tokenStoreLoaded ?? 0}/
									{settingsQuery.data?.tokenStoreExpected ?? 12}
								</Badge>
							</div>
							<p className="text-xs text-muted-foreground">
								Supabase يحتفظ بـSHA-256 وtoken hint فقط لأغراض المطابقة
								والتدقيق.
							</p>
						</CardContent>
					</Card>
					<Card className="shadow-card">
						<CardHeader>
							<CardTitle className="text-base flex items-center gap-2">
								<Activity className="h-4 w-4" /> Connection Semantics
							</CardTitle>
						</CardHeader>
						<CardContent className="space-y-3">
							{[
								["Online", "آخر MCP activity خلال 10 دقائق"],
								["whoami", "يسجل last_whoami_at كاختبار هوية واتصال"],
								["Offline", "يُستنتج تلقائيًا عند تقادم heartbeat"],
							].map(([title, text]) => (
					...
					<DialogContent className="max-w-3xl" dir="rtl">
						<DialogHeader>
							<DialogTitle className="flex items-center gap-2">
								<UserRoundCog className="h-5 w-5" /> إدارة {selected?.id}
							</DialogTitle>
							<DialogDescription>
								تفاصيل الهوية، Token، واختبار إرسال حقيقي من صندوق الوكيل.
							</DialogDescription>
						</DialogHeader>
						{selected && (
							<Tabs defaultValue="identity" className="space-y-4">
								<TabsList>
									...
									<TabsTrigger value="test">اختبار SMTP</TabsTrigger>
								</TabsList>
								<TabsContent value="identity" className="space-y-4">
									<div className="grid sm:grid-cols-2 gap-3">
										{[
											...
											["Token hint", selected.tokenHint ?? "—"],
											["Password source", selected.smtpPasswordSource],
											["Connection", selected.connection?.status ?? "offline"],
										].map(([label, value]) => (
											<div key={label} className="rounded-lg border p-3">
												...
												<div className="flex items-center justify-between gap-2 mt-1">
													...
													<Button
														...
														size="icon"
														className="h-7 w-7"
														onClick={() => void copyValue(String(value), label)}
													>
									...
									<div className="grid grid-cols-3 gap-3 text-center">
										<div className="rounded-lg bg-muted/40 p-3">
											<p className="text-lg font-bold">
												{selected.recommendedTemplateCount}
											</p>
											<p className="text-[10px] text-muted-foreground">
												Recommended
											</p>
										...
										<div className="rounded-lg bg-muted/40 p-3">
											<p className="text-lg font-bold">
												{selected.availableTemplateCount}
											</p>
											<p className="text-[10px] text-muted-foreground">
												Available
											</p>
								...
								<TabsContent value="token" className="space-y-4">
									<Alert>
										...
										<AlertTitle>Rotate returns the clear token once</AlertTitle>
										<AlertDescription>
											بعد التدوير يجب تحديث Secret الخاص باتصال MCP في Foundry.
											التوكن الجديد يُكتب إلى volume قبل مزامنة hash إلى
											Supabase.
										</AlertDescription>
									</Alert>
									{tokenResult ? (
										<div className="rounded-xl border p-4 space-y-3">
											<Label>New bearer token</Label>
											<div className="flex gap-2">
												...
												<Button
													...
													size="icon"
													onClick={() =>
														void copyValue(tokenResult.token, "Token")
													}
												>
