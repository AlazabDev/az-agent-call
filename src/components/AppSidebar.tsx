...
export function AppSidebar() {
	...
	const renderItems = (items: typeof operations) =>
		items.map((item) => (
			<SidebarMenuItem key={item.url}>
				<SidebarMenuButton asChild isActive={location.pathname === item.url}>
					<NavLink
						...
						activeClassName="bg-sidebar-accent text-white font-medium border-r-2 border-[#FFB900]"
					>
		...
		<Sidebar collapsible="icon" side="right">
			<SidebarHeader className="p-4 border-b border-sidebar-border">
				<div className="flex items-center gap-3">
					<div className="rounded-xl p-2.5 flex-shrink-0 bg-[#FFB900] text-[#030957]">
						<PhoneCall className="h-5 w-5" />
					</div>
					{!collapsed && (
						<div className="text-right">
							<h2 className="text-sm font-bold text-white">Az Agent Call</h2>
							<p className="text-[11px] text-sidebar-foreground">
								Alazab Agent Contact Center
							</p>
						</div>
					)}
				</div>
			...
			<SidebarContent>
				...
				<SidebarGroup>
					...
					<SidebarGroupContent>
						<SidebarMenu>{renderItems(engineering)}</SidebarMenu>
					</SidebarGroupContent>
				</SidebarGroup>
			</SidebarContent>
		</Sidebar>
