	...
	return (
		...
		>
			...
			>
				<div
					className={
						"px-3.5 py-2.5 rounded-2xl shadow-bubble text-sm leading-relaxed " +
						(isUser
						...
						<div className="mt-3 flex flex-wrap gap-2">
							{msg.buttons.map((button, index) => (
								<button
									...
									type="button"
									onClick={() =>
										handleButtonClick(button.title, button.payload, button.url)
									}
				...
				{!isUser && msg.content && ttsEnabled && !msg.isError && (
					<button
						onClick={onSpeak}
						className="text-xs text-muted-foreground hover:text-brand flex items-center gap-1 transition-colors px-1"
				...
				>
					{new Date(msg.ts).toLocaleTimeString("ar-EG", {
						hour: "2-digit",
						minute: "2-digit",
					})}
				</time>
