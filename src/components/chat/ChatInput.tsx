...
export function ChatInput({
	...
	const handleFiles = (list: FileList | null) => {
		...
		const oversize = arr.filter(
			(f) => f.size > CONFIG.chat.maxFileSizeMB * 1024 * 1024,
		);
		if (oversize.length > 0) {
			toast.error(
				`بعض الملفات تتجاوز ${CONFIG.chat.maxFileSizeMB} MB وسيتم تجاهلها.`,
			);
		}
		const valid = arr
	...
	const canSend =
		!streaming && (value.trim().length > 0 || pendingFiles.length > 0);
			...
			{pendingAttachments.length > 0 && (
				<div className="px-3 py-2 flex flex-wrap gap-2 border-b border-border bg-muted/30">
					{pendingAttachments.map((a, i) => (
						<div
							key={i}
							className="flex items-center gap-1.5 bg-card border border-border rounded-lg px-2 py-1 text-xs max-w-[180px]"
						>
							<button
