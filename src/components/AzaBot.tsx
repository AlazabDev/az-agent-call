...
const LAUNCHER_PHRASES = ["أهلاً بك 👋", "أنا عزبوت", "جاهز أساعدك"];
...
export default function AzaBot() {
	...
	// Hooks
	const { messages, streaming, send, clearMessages, downloadConversation } =
		useChat(site.id, modeState.mode);
	const tts = useTTS();
	...
	}, [messages, streaming, tab, tts.speak]);
		...
		return (
          ...
          `}
				...
				>
					...
					<span className="absolute -bottom-1 left-1/2 h-3 w-3 -translate-x-1/2 rotate-45 border-b border-r border-slate-200 bg-white/95" />
				</div>
