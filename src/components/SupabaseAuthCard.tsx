...
async function loadEdgeStatus(): Promise<EdgeStatus> {
  const { data, error } = await supabase.functions.invoke("agent-call-status", { method: "GET" });
    ...
    <Card className="shadow-card">
      ...
      <CardContent className="space-y-4">
        <div className="grid sm:grid-cols-2 gap-3 text-sm">
          ...
          <div className="rounded-lg border bg-muted/30 p-3"><p className="text-xs text-muted-foreground">Edge Function</p><p className="mt-1 text-xs font-medium">agent-call-status · JWT required</p></div>
        </div>
        {!compact && <>
          <Separator />
