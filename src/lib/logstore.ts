...
const LOG_PATH = process.env.SEND_LOG_PATH ?? path.join(process.cwd(), 'data', 'send-log.json');
...
function load(): LogEntry[] {
  ...
  try {
    ...
    console.error(`[az-agent-mail] failed to parse ${LOG_PATH}, starting a fresh log:`, error);
    return [];
  }
}
