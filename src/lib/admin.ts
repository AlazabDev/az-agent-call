...
if (!ADMIN_PASSWORD) {
	console.error(
		"[az-agent-call] WARNING: ADMIN_PASSWORD is not set — the /admin dashboard is disabled until you set it.",
	);
}
...
adminRouter.use((req, res, next) => {
	...
	if (password !== ADMIN_PASSWORD) {
		res.set("WWW-Authenticate", 'Basic realm="az-agent-call admin"');
...
adminRouter.post("/api/settings/smtp", (req, res) => {
	...
	if (!host || !Number.isInteger(port) || port <= 0) {
		res.status(400).json({
			error:
				'Body must include a non-empty "host" and a positive integer "port".',
		});
		return;
	}
	setSmtp(host, port);
...
adminRouter.post("/api/agents/:id/password", (req, res) => {
	...
	if (!password) {
		res
			.status(400)
			.json({ error: 'Body must include a non-empty "password".' });
		return;
	}
...
adminRouter.post("/api/agents/:id/test-send", async (req, res) => {
	...
	const to =
		typeof req.body?.to === "string" && req.body.to ? req.body.to : agent.email;

	try {
		const result = await sendAsAgent(agent, {
			to,
			subject: `Test send from ${agent.email}`,
			text: `This is a test email sent from the az-agent-call dashboard at ${new Date().toISOString()}.`,
		});
