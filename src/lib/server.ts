...
export function buildServerForAgent(agent: AgentConfig): McpServer {
  const server = new McpServer(
    {name: 'az-agent-mail', version: '0.1.0'},
    {
      capabilities: {tools: {}},
      instructions: [
  ...
  server.registerTool(
    'send_email',
    {
      ...
      inputSchema: {
        to: recipients.describe('Recipient email address, or an array of them.'),
        cc: recipients.optional(),
        bcc: recipients.optional(),
        subject: z.string().min(1).max(998),
        text: z.string().optional().describe('Plain-text body. Provide this and/or `html`.'),
        html: z.string().optional().describe('HTML body. Provide this and/or `text`.'),
    ...
    async ({to, cc, bcc, subject, text, html, replyTo}) => {
      if (!text && !html) {
        return {
          isError: true,
          content: [{type: 'text', text: 'Provide `text` and/or `html` for the email body.'}],
        };
      }

      try {
        const result = await sendAsAgent(agent, {to, cc, bcc, subject, text, html, replyTo});
  ...
  server.registerTool(
    ...
    async () => ({content: [{type: 'text', text: JSON.stringify({email: agent.email, foundryId: agent.foundryId})}]}),
  );
