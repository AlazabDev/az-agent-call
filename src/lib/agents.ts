...
const TOKENS_PATH =
	...
	path.join(process.cwd(), "data", "agent-tokens.json");

function loadOrCreateTokens(): Record<string, string> {
	if (existsSync(TOKENS_PATH)) {
		return JSON.parse(readFileSync(TOKENS_PATH, "utf8")) as Record<
			string,
			string
		>;
	}
	...
	console.error(
		`[az-agent-call] generated ${AGENT_IDS.length} bearer tokens at ${TOKENS_PATH} — ` +
...
const tokenIndex = new Map<string, AgentConfig>(
	Object.values(AGENTS).map((a) => [a.bearerToken, a]),
);
