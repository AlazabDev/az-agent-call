...
function load(): Settings {
	if (existsSync(SETTINGS_PATH)) {
		try {
			...
			console.error(
				`[az-agent-call] failed to parse ${SETTINGS_PATH}, falling back to defaults:`,
				error,
			);
		}
	}
...
const DEFAULT_PASSWORD_SUFFIX =
	process.env.MAILBOX_PASSWORD_DEFAULT_SUFFIX ?? "202555";
...
export function isPasswordOverridden(agentId: string): boolean {
	return Boolean(
		settings.passwords[agentId] ||
			process.env[`MAILBOX_PASSWORD_${agentId.toUpperCase()}`],
	);
}
