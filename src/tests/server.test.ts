...
Deno.test("health endpoint is public", async () => {
	const response = await createHandler(config())(
		new Request("http://local/healthz"),
	);
...
Deno.test("template API rejects missing token", async () => {
	const response = await createHandler(config())(
		new Request("http://local/v1/templates"),
	);
...
Deno.test("render API returns escaped HTML", async () => {
	const response = await createHandler(config())(
		new Request("http://local/v1/render", {
			method: "POST",
			headers: {
