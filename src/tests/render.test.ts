...
Deno.test("renders a production template", () => {
	...
	if (!output.subject.includes("MS-2026-0042"))
		throw new Error("subject reference missing");
...
Deno.test("escapes untrusted values", () => {
	...
	if (output.html.includes("<script>"))
		throw new Error("unsafe HTML was not escaped");
});
