...
if (TEMPLATE_COUNT !== expected) {
	throw new Error(
		`Template count mismatch: expected ${expected}, got ${TEMPLATE_COUNT}`,
	);
}
...
for (const id of AGENT_IDS) {
	...
	if (!(foundry in counts))
		throw new Error(`Missing agent identity in catalog: ${foundry}`);
}
...
for (const row of rows) {
	...
	if (!result.subject || !result.html || !result.text)
		throw new Error(`Empty rendered content: ${row.id}`);
...
console.log(
	JSON.stringify(
		{ ok: true, templates: TEMPLATE_COUNT, rendered, byAgent: counts },
		null,
		2,
	),
);
