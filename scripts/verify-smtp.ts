...
const results: Array<{ agentId: string; ok: boolean; source: string; error?: string }> = [];

for (const id of AGENT_IDS) {
  const agent = await getAgent(id);
  if (!agent) {
    results.push({ agentId: id, ok: false, source: "missing", error: "Agent not found in Supabase" });
    continue;
  }
