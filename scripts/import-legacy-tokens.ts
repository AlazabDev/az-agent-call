...
if (source !== config.agentTokensPath) await copyFile(source, config.agentTokensPath);
const result = await initializeAgentTokens();
console.log(`Imported ${AGENT_IDS.length} agent tokens into ${result.path}; generated=${result.generated.length}`);
