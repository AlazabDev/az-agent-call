...
Deno.serve(async (req) => {
  ...
  const [{ data: platformRoles, error: platformError }, { data: delegated, error: delegatedError }] = await Promise.all([
    ...
    admin.from("call_admins").select("role").eq("user_id", userData.user.id).maybeSingle(),
  ]);
  ...
  const role = roles.has("platform_owner")
    ...
    : roles.has("platform_admin")
      ...
      : delegated?.role === "operator" || delegated?.role === "viewer"
        ? delegated.role
        : null;
  if (!role) return json({ error: "Az Agent Call Center access required" }, 403);

  const since = new Date(Date.now() - 86_400_000).toISOString();
  const [agents, templates, sent, failed, gateways, connections] = await Promise.all([
    admin.from("call_agents").select("id", { count: "exact", head: true }).eq("enabled", true),
    admin.from("call_templates").select("id", { count: "exact", head: true }).eq("enabled", true),
    admin.from("call_logs").select("id", { count: "exact", head: true }).eq("status", "success").gte("created_at", since),
    admin.from("call_logs").select("id", { count: "exact", head: true }).eq("status", "failed").gte("created_at", since),
    admin.from("call_gateway_instances")
      .select("instance_id,hostname,version,status,started_at,last_seen_at,smtp_host,smtp_port,template_count")
      .order("last_seen_at", { ascending: false })
      .limit(5),
    admin.from("call_agent_connections")
