...
Deno.serve(async (req) => {
	...
	const [
		{ data: platformRoles, error: platformError },
	...
	const role = roles.has("platform_owner")
		...
		: roles.has("platform_admin")
			...
			: delegated?.role === "operator" || delegated?.role === "viewer"
				...
				: null;
	if (!role)
		return json({ error: "Az Agent Call Center access required" }, 403);

	const since = new Date(Date.now() - 86_400_000).toISOString();
	const [agents, templates, sent, failed, gateways, connections] =
		await Promise.all([
			admin
				.from("mail_agents")
